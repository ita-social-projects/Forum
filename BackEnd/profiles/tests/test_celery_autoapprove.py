...
class TestCelery(APITestCase):
    ...
    def test_autoapprove_banner_and_logo(self, mock_task): 
        ...
        mock_task.assert_called_with((self.profile.id, self.banner.uuid, self.logo.uuid), countdown=AnyInt())  
    ...
    def test_autoapprove_only_banner(self, mock_task):
        ...
        mock_task.assert_called_with((self.profile.id, self.banner.uuid, None), countdown=AnyInt()) 
    ...
    def test_autoapprove_only_logo(self, mock_task):
        ...
        mock_task.assert_called_with((self.profile.id, None, self.logo.uuid), countdown=AnyInt()) 
    ...
    def test_change_autoapprove_time(self, mock_task):
        ...
        mock_task.assert_called_with((self.profile.id, self.banner.uuid, self.logo.uuid), countdown=test_value*60*60)        