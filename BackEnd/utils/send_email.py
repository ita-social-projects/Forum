...
def send_moderation_email(profile):
    ...
    if manager.check_for_moderation():
        ...
        banner = profile.banner if profile.banner != profile.banner_approved else False
        logo = profile.logo if profile.logo != profile.logo_approved else False
        approve_url = None
        reject_url = None
        context = {
        ...
        email = EmailMultiAlternatives(
            subject=f"{profile.name} - {update_date}: Запит на затвердження змін в обліковому записі компанії",
            body=email_body,
