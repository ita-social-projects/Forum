...
class TestProfileDetailAPIView(APITestCase):
    ...
    def test_get_profile_unauthorized(self):
        self.client.logout()
        response = self.client.get(f"/api/profiles/{self.user1_id}")
        self.assertEqual(401, response.status_code)

    def test_delete_profile_unauthorized(self):
        self.client.logout()
        response = self.client.delete(f"/api/profiles/{self.user1_id}")
        self.assertEqual(401, response.status_code)
    ...
    def test_get_profile_of_other_user(self):
        response = self.client.get(f"/api/profiles/{self.user2_id}")
        self.assertEqual(200, response.status_code)
    ...
    def test_delete_profile_of_other_user_authorized(self):
        response = self.client.delete(f"/api/profiles/{self.user2_id}")
        self.assertEqual(404, response.status_code)
