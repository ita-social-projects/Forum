...
from apps.profiles.factories import SavedCompanyFactory, SavedStartupFactory
...
class TestProfileFilterCompanyType(APITestCase):
    ...
    def test_get_profiles_unauthorized_content(self):
        ...
        self.assertCountEqual(
            [self.startup.id, self.company.id], ids_from_response
        )
    ...
    def test_get_profiles_filter_companies_authorized_count(self):
        ...
        self.assertEqual(
            ...
            msg="Total result amount doesn't match.",
        )
        self.assertEqual(
            1,
            len(
    ...
    def test_get_profiles_filter_companies_authorized_content(self):
        ...
        all_companies_ids_from_response = [
            company["id"] for company in response.data["results"]
        ]

        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(
            saved_company.company.id,
            saved_from_response[0]["id"],
            msg="Saved companies do not match",
        )
        self.assertCountEqual(
            all_companies_ids_from_db,
            all_companies_ids_from_response,
    ...
    def test_get_profiles_filter_startups_authorized_content(self):
        ...
        all_startups_ids_from_response = [
            startup["id"] for startup in response.data["results"]
        ]

        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(
            saved_startup.company.id,
            saved_from_response[0]["id"],
            msg="Saved startups do not match",
        )
        self.assertCountEqual(
            all_startups_ids_from_db,
            all_startups_ids_from_response,
...
class TestProfileFilterActivityType(APITestCase):
    def setUp(self) -> None:
        ...
        self.importers = ProfileCompanyFactory.create_batch(
            size=self.importers_number, activities=(self.importer_activity,)
        )
        self.producers = ProfileStartupFactory.create_batch(
            size=self.producers_number, activities=(self.producer_activity,)
        )
        self.retailers = ProfileCompanyFactory.create_batch(
            size=self.retailers_number, activities=(self.retail_activity,)
        )
    ...
    def test_get_profiles_any_user_filter_producer_content(self):
        ...
        producers_from_db = Profile.objects.filter(
            activities=self.producer_activity, is_deleted=False
        ).order_by("id")
        self.assertEqual(producers_from_db.count(), self.producers_number)
        self.assertTrue(
            all(
                [
                    self.producer_activity.id
                    in [
                        item.get("id")
    ...
    def test_get_profiles_any_user_filter_importer_content(self):
        ...
        importers_from_db = Profile.objects.filter(
            activities=self.importer_activity, is_deleted=False
        ).order_by("id")
        self.assertEqual(importers_from_db.count(), self.importers_number)
        self.assertTrue(
            all(
                [
                    self.importer_activity.id
                    in [
                        item.get("id")
                        for item in response.data["results"][i]["activities"]
                    ]
    ...
    def test_get_profiles_any_user_filter_retail_content(self):
        ...
        retailers_from_db = Profile.objects.filter(
            activities=self.retail_activity, is_deleted=False
        ).order_by("id")
        self.assertEqual(retailers_from_db.count(), self.retailers_number)
        self.assertTrue(
            all(
                [
                    self.retail_activity.id
                    in [
                        item.get("id")
                        for item in response.data["results"][i]["activities"]
                    ]
    ...
    def test_get_profiles_any_user_filter_horeca_content(self):
        ...
        horecas_from_db = Profile.objects.filter(
            activities=self.horeca_activity, is_deleted=False
        ).order_by("id")
        self.assertEqual(horecas_from_db.count(), self.horecas_number)
        self.assertTrue(
            all(
                [
                    self.horeca_activity.id
                    in [
                        item.get("id")
                        for item in response.data["results"][i]["activities"]
                    ]
