...
from apps.profiles.factories import ProfileStartupFactory, ViewedCompanyFactory
...
class TestViewedCompanyAPI(APITestCase):
    ...
    def test_create_viewed_company_authorized(self):
        ...
        response = self.client.post(
            ...
            data={"user": self.user.id, "company": profile.id},
        )
    ...
    def test_create_viewed_company_authorized_own_company(self):
        ...
        response = self.client.post(
            path="/api/viewed-list/",
            data={"user": self.user.id, "company": own_profile.id},
        )
